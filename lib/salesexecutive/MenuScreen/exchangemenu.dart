...
class _ExchangemenuState extends State<Exchangemenu> {
  ...
  bool exchangepending = true;
  bool exchangefinalize = false;
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
                    ...
                    child: RawMaterialButton(
                      onPressed: () {
                        setState(() {
                          ...
                          exchangefinalize = true;
                        });
                      },
                      constraints: BoxConstraints(),
                      fillColor: exchangefinalize ? kred : kwhite,
                      padding: EdgeInsetsGeometry.symmetric(horizontal: SizeConfig.w(15),vertical: SizeConfig.h(8)),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadiusGeometry.circular(10),
                        side: BorderSide(
            ...
            if(exchangepending == true)
            Expanded(
              child: Exchangepending()
            ),
            if(exchangefinalize == true)
            Expanded(
              child: Exchangefinalize()
            )
          ],
        )
      ),
...
class Exchangefinalize extends StatefulWidget {
  const Exchangefinalize({super.key});

  @override
  State<Exchangefinalize> createState() => _ExchangefinalizeState();
}

class _ExchangefinalizeState extends State<Exchangefinalize> {
  ...
  Future<void> _fetchPage({String? url}) async {
    try {
      final Uri uri = Uri.parse(url ?? 'https://app.pravinhonda.com/api/exchange-finalized');
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ExchangeMenu(
                          ...
                          presentmodelowned: data['present_model_owned']?.toString() ?? '',
                          expectedprice: data['expected_price']?.toString() ?? '',
                          onTap: () {
                            Navigator.push(
                              context,
...
class Exchangepending extends StatefulWidget {
  const Exchangepending({super.key});

  @override
  State<Exchangepending> createState() => _ExchangependingState();
}

class _ExchangependingState extends State<Exchangepending> {
  List<dynamic> _allData = [];
  List<dynamic> alldata = [];
  bool isLoading = true;
  bool isLoadingMore = false;
  ...
  Future<void> _fetchPage({String? url}) async {
    try {
      final Uri uri = Uri.parse(url ?? 'https://app.pravinhonda.com/api/exchange-pending');
